...
const LibraryService_1 = require("./application/LibraryService");
const LibraryCLI_1 = require("./presentation/LibraryCLI");
const repositories_1 = require("./infrastructure/repositories");
// ── Шаардлага 8: BookCategory (aggregation)
const categories = [
    new models_1.BookCategory(1, "Уран зохиол"),
    new models_1.BookCategory(2, "Технологи"),
    new models_1.BookCategory(3, "Шинжлэх ухаан"),
];
// ── Repository-үүдийг үүсгэж өгөгдөл дүүргэнэ
const bookRepo = new repositories_1.InMemoryBookRepository();
const memberRepo = new repositories_1.InMemoryMemberRepository();
const loanRepo = new repositories_1.InMemoryLoanRepository();
// ── Шаардлага 3: Домайн ангиудыг үүсгэнэ (Book)
const seedBooks = [
    new models_1.Book(1, "Нууцлаг Газар", "Д.Нямаа", 1),
    new models_1.Book(2, "Python Guide", "Т.Болд", 2),
    new models_1.Book(3, "Монголын түүх", "Б.Лхамсүрэн", 3),
    new models_1.Book(4, "Clean Code", "Robert Martin", 2),
    new models_1.Book(5, "Одны хот", "Д.Урнаа", 1),
];
seedBooks.forEach((b) => {
    var _a;
    bookRepo.save(b);
    (_a = categories.find((c) => c.id === b.categoryId)) === null || _a === void 0 ? void 0 : _a.addBook(b.id);
});
// ── Шаардлага 3: Домайн ангиудыг үүсгэнэ (Member)
const seedMembers = [
    new models_1.Member(1, "Батбаяр Д."),
    new models_1.Member(2, "Оюунцэцэг М."),
    new models_1.Member(3, "Гантулга Б."),
];
seedMembers.forEach((m) => memberRepo.save(m));
// ── Application layer болон Presentation layer-ийг холбоно
const service = new LibraryService_1.LibraryService(bookRepo, memberRepo, loanRepo, categories);
const cli = new LibraryCLI_1.LibraryCLI(service);
// ============================================================
